...
**For the existing server and ordinary Git-pull updates, follow [UPDATE_DEPLOYMENT.md](UPDATE_DEPLOYMENT.md).** It covers both updating the permanent checkout at `/srv/huddleston/app` and the one-time conversion from an extracted package, keeping `/srv/huddleston/current` as the service path. Check the current symlink target to choose the appropriate procedure. The release-directory workflow below remains an alternative, not a required step for each update.
...
For actual disaster recovery, provision a fresh PostgreSQL database/role, restore a chosen trusted dump with `pg_restore --exit-on-error --no-owner --no-privileges --dbname=<new-database> <backup>.dump` using a protected password file or environment (not a command argument), and point a private maintenance instance at it. Restore the saved application secret, matching release, and configuration. Verify row checksums, migrations, login/password reset, saved Study sessions and completions, previous practice results, and administrator protection before changing the live database setting. Clear restored login sessions with `clearsessions` only for expired sessions; to invalidate **all** restored browser sessions, delete `django_session` rows through a trusted maintenance command. Keep the failed database and newest backups until recovery is confirmed.
...
3. Collect static assets, switch `current` to the new release, then restart `huddleston`. Check HTTPS, login, the Study reading/quiz/review/pass flow, Progress, Explore, history, and timers. Never run production database tests against the live database.
...
5. The redesign adds migration 0007. Older code does not understand Study sessions or completions; do not roll back to it after new records exist. Prefer a forward fix. Reversing 0007 removes saved Study evidence. Restore only with a matching release and an explicit decision about progress recorded since the backup.
...
Start with 4–6 students and pseudonymous accounts. Keep any real-name mapping outside the site. Use a 15–20 minute session to check temporary-password replacement, choosing interests, reading topics, typed quizzes, targeted rereading/retries, saved history, and resuming on another device. Verify that passes count once in Progress and browsing Explore does not grant completion. Check keyboard use, small phones, short sessions, and reduced-motion preferences.
